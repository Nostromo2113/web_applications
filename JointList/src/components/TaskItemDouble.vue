            ...
            <Collapse :when="taskFullWindow || isOpen" class="my-class">
                <div class="task-elems__wrapper">
                    <div class="taskElems" :class="taskFullWindow ? 'taskElems-full-window' : ''"
                        v-for="el in taskItems.list" :key="el.id">
                        ...
                        <div class="redaction-element" v-else>
                            ...
                            <my-input v-focus style="padding: 4px; 
                            border-radius: 0; 
                            box-shadow: none; 
                            border:none; 
                            border-bottom: 0.1px dotted silver;" v-model="redactionElement"
                                @click="redactionElement = el.item" :placeholderText4="el.item" :is="4" />
                        </div>
                    </div>
                </div>
                <button class="task__element" @click="createTaskElement(taskItems)">✚</button>
            </Collapse>
            <button-item style="right: 50%; transform: translateX(50%);" v-if="!taskFullWindow"
